...
def numlist(): #生成两个列表：员工放号和电信返佣金号码列表
    ...
    dianxin_wb.close()
    app.quit()
    print(yuangong_numlist)
    print(dianxin_numlist)
numlist()
...
def new_excel(): #生成新excel文件
    ...
    for i in dianxin_numlist:
        if i in correct_list:
            dianxinyongjin_list.append(dianxin_wb.sheets['sheet1'].range('B'+str(dianxin_numlist.index(i)+3)).value) #生成佣金列表
    for i in chongfunum_yuangong_list2:
        new_wb.sheets['sheet1'].range(i).value=dianxinyongjin_list[chongfunum_yuangong_list2.index(i)] #将佣金填入G range
    print(chongfunum_yuangong_list2)
    print(dianxinyongjin_list)
